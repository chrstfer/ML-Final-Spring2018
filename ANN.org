...
#+PROPERTY: header-args :session ANNimpl :exports both
...
*** Sandbox/Toy Example
#+BEGIN_SRC R :results output graphics :file imgs/tests/learn1.png 
   #rm(list=ls())
  set.seed(1)
...
#+RESULTS:
[[file:imgs/tests/learn1.png]]



*** Model
    The below isn't especially functional. The intent is to produce a function
    of similar capacity to other available modelling functions such as knn,
    naiveBayes, and svm. The end result would be a library of functions as
    detailed in the above todo. 

    This was a difficult prospect to complete in a week. The above toy example
    is quite slow, with a noticable pause just to train the network to recognize
    one image. Below we tried to fix some of these issues, attempting to limit
    the amount of memory copying and reduce the total number of commands executed. 
...
* Analysis
  A previous version was producing (abysmal) results. This could have actually
  been a problem with the given network structure. This version still exists in
  git,
...
* Conclusion
  ...
  implement the features we initially envisioned. We have a semi-functional toy
  example that trains on one image, proving our feedforward/backprop
  works. However, turning this into a functional package for arbitrary
  Multi-layer Perceptron networks was a difficult endeavour. Though we had
  reasonable success with the single digit yes or no problem, our results at a
  producing a single network for multiple digit were lackluster at best. 

  That being said, 
  
  
  We plan on continuing to develop this project, hoping to produce a toy library
  capable of supporting arbitrary

** Followup Questions
- how can we optimize? What performance features does R have?
  - Intel GLAS?
- Does R have any kind of recursion optimizations? If it does tail-call
  recursion, or something similar could be implemented with environments (maybe
  a C/++ module? if we're feeling ambitious), we might be able to simplify this
  code while simultaneously preventing a ton of memory copying.
  
* Sources/Bibliography
  We read these in the process of completing this project, mostly consulting the
  math derivations, especially when attempting to write the single digit toy
  example, and then again when attempting to convert that to an R class that can
  handle arbitrary networks.

  Also consulted were more resources on the R language than I kept track of, but
  the website [[https://www.r-bloggers.com/][R Bloggers]] was especially useful, as were the built in docs.
